  ...
  return (
    <>
      ...
      <main className="max-w-5xl mx-auto px-4 py-10">
        ...
        <div className="bg-white rounded-2xl shadow p-6 mb-6">
          ...
          {profile.cvUrl ? (
            <div className="flex gap-4">
              {/* VOIR LE CV */}
              <a
                href={`/researcher/public/cv-viewer?userId=${encodeURIComponent(profile.id)}`}
                target="_blank"
        ...
        {/* Cours */}
        {profile.cours && profile.cours.length > 0 && (
          <div className="bg-white rounded-2xl shadow p-6 mb-6">
            <h2 className="text-xl font-semibold mb-4">
              📚 {language === 'fr' ? 'Cours' : 'Courses'}
            </h2>
            <div className="space-y-3">
              {profile.cours.map((course) => (
                  ...
                  {course.curricula && (
                    <p className="text-sm text-gray-500 mt-1">
                      <span className="font-medium">
                        {language === 'fr' ? 'Programme' : 'Curriculum'} :
                      </span>{' '}
                      {course.curricula}
                    </p>
                  )}
        ...
        {/* Médias */}
        {profile.media_artefacts && profile.media_artefacts.length > 0 && (
          <div className="bg-white rounded-2xl shadow p-6 mb-6">
            <h2 className="text-xl font-semibold mb-4">
              🎬 {language === 'fr' ? 'Médias' : 'Media'}
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {profile.media_artefacts.map((media) => (
        ...
        {/* Distinctions */}
        {profile.distinctions && profile.distinctions.length > 0 && (
          <div className="bg-white rounded-2xl shadow p-6 mb-6">
            <h2 className="text-xl font-semibold mb-4">🏆 {t('distinctions_title', language) || 'Distinctions'}</h2>
            <div className="space-y-3">
              {profile.distinctions.map((distinction) => (
                <div key={distinction.id} className="border-l-4 border-yellow-600 pl-4">
                  <p className="font-semibold text-gray-800">
                    {distinction.title} <span className="text-gray-500 font-normal">({distinction.year})</span>
                  </p>
